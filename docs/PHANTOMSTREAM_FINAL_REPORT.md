...
graph TD
    ...
    subgraph Phase7_Red [Phase 7 红队深度探测 - 完胜]
        R7_DeepScan[深度扫描工具] -->|发现并验证SMask锚点| B7_SMaskStream
        R7_DeepScan -->|理解XRef流| R7_XRef[对象 74: XRef流变化]
        R7_XRef -->|确认标准结构| R7_NoChallenge[无额外校验机制]
        style R7_DeepScan fill:#9cf,stroke:#333,stroke-width:2px
        style R7_XRef fill:#ff9,stroke:#333,stroke-width:2px
        style R7_NoChallenge fill:#6f6,stroke:#0f0,stroke-width:2px
        style B5_Stream stroke-dasharray: 5 5;
        style B7_AttachStream stroke-dasharray: 5 5;
        style B7_SMaskStream stroke-dasharray: 5 5;
    end
    Note right of R7_DeepScan: 主签名被清除，SMask作为备份有效
```
