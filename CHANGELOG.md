...
- **CLI 优化**：`sign` 和 `verify` 命令的 `--key` 参数现在是可选的。如果未提供，将自动使用环境变量 `DEFAULT_KEY`。
...
### ✨ 新增
- **Visual 锚点自适应字体**：根据水印内容长度自动调整字体大小（20-48pt），避免长文本截断。
- **默认启用 Visual 锚点**：Sign 命令默认签名策略中加入 Visual 视觉水印，增强震慑力。

### 🔧 优化
- **Visual 水印格式调整**：移除默认的“机密文件 |”前缀，仅显示用户输入的消息内容。
- **修复注入链 Bug**：修复在多锚点注入过程中因临时文件重名导致的潜在错误。
